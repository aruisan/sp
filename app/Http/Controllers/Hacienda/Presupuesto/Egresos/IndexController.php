                ...
                foreach ($ordenPagos as $data){
                    ...
                    if ($pagoFind->count() == 1){
                        ...
                        foreach ($oPago->rubros as $rubroOP){
                            ...
                            if ($rubroOP->orden_pago->estado == "1") {
                                if (isset($rubroOP->cdps_registro->fontRubro)){
                                    $valoresPagos[] = ['id' => $rubroOP->cdps_registro->fontRubro->rubro->id, 'val' => $rubroOP->valor, 'code' => $rubroOP->cdps_registro->fontRubro->rubro->plantilla_cuipos_id];
                                }
                            }
                        }
                        ...
                        foreach ($rubro->first()->fontsRubro as $itemFont){
                            ...
                            if (count($itemFont->dependenciaFont) > 0){
                                foreach ($itemFont->dependenciaFont as $depFont){
                                    ...
                                    if ($PDef > 0 or array_sum($valueRubrosCCred) == array_sum($value) or array_sum($valueRubrosCCred) == array_sum($valueRubrosAdd)){
