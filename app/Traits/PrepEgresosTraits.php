            ...
            if ($data->id < '324') {
                ...
                if ($data->id == '318') {
                    ...
                    foreach ($otherRubs as $other) {
                        ...
                        if($rubroOtherFind->first()) {
                            foreach ($rubroOtherFind as $rubOtherFindFor){
                                if($rubOtherFindFor->fontsRubro){
                                    foreach ($rubOtherFindFor->fontsRubro as $fuenteRubro){
                                ...
                                if(count($rubOtherFindFor->rubrosMov) > 0){
                                    foreach ($rubOtherFindFor->rubrosMov as $mov){
                                        if ($mov->valor > 0 ){
                                            if ($inicio != null){
                                                if (date('Y-m-d', strtotime($mov->created_at)) <= $final and date('Y-m-d', strtotime($mov->created_at)) >= $inicio){
                                                    if ($mov->movimiento == "1") {
                ...
                if ($key == false) {
                    ...
                    if($otherRubs and $oldCode != null) {
                        foreach ($otherRubs as $other) {
                            $rubroOtherFind = Rubro::where('vigencia_id', $vigencia_id)->where('plantilla_cuipos_id', $other->id)->get();
                            if($rubroOtherFind->first()) {
                                foreach ($rubroOtherFind as $itemROF){
                                    $exit = false;
                                    if($itemROF->fontsRubro){
                                        foreach ($itemROF->fontsRubro as $fuenteRubro) {
                                            $valueRubros[] = $fuenteRubro->valor;
                                            ...
                                            foreach ($itemROF->rubrosMov as $mov){
                                                if ($mov->valor > 0 ){
                                                    if ($mov->movimiento == "2") {
                ...
                if($rubro->first()->fontsRubro){
                    ...
                    $exit = false;
                    foreach ($rubro as $dataRub){
                        foreach ($dataRub->fontsRubro as $itemFont){
                            //VALIDACION PARA LAS ADICIONES Y REDUCCIONES
                ...
                //el ID 783 no es necesario en el presupuesto 2024
