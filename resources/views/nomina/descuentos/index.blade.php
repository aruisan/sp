...
<ul class="nav nav-pills">
    <li class="nav-item">
		<a class="nav-link" href="{{route('nomina.'.$tipo.'s.index')}}"> {{ucfirst($tipo)}}s</a>
	</li>
...
<div class="tab-content" style="background-color: white">
	<div id="lista" class="tab-pane active"> <div class="breadcrumb text-center">
		<strong>
			<h3><b>Nomina de Descuentos</b></h3>
		</strong>
	</div>
						...
						<table class="table table-bordered cell-border table-hover" id="example"  data-form="deleteForm">
							...
							@foreach($nominas as $key => $nomina)
								<tr>
									<td>{{$key+1}}</td>
									<td>{{$nomina->mes}}</td>
									<td>${{number_format($nomina->empleados_nominas->sum('descuentos_total'), 2)}}</td>
									<td>
										<a href="{{route('nomina-descuentos.show', $nomina->id)}}" class="btn btn-sm btn-primary" title="ver nomina de descuentos">
											<i class="fa fa-eye" aria-hidden="true"></i>
										</a>
										@if(!$nomina->descuentos)
